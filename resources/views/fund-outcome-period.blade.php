...
<section class="clearfix section page-news bg-gray pt-5" style="border-top: 2px solid #D7D7D7;">
    <div class="container anim">
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            <tr class="nobg subhead-row">
                                ...
                                <th scope="col" class="text-center text-nowrap">Expected Upside Participation</th>
        ...
        if(($fund['display_current_outcome_period_values'] == true) && !empty($daily_nav_usbank)){
            ...
            $ETF_CURRENT_NAV = (float)$daily_nav_usbank->market_price;
            $YTD_SP_RETURN = 0;
            $INDEX_RETURN = 0;
            $OUTCOME_START_DATE = '2020-01-01';
            $OUTCOME_END_DATE = '2021-01-01';

            $PERIOD_RETURN = ($ETF_CURRENT_NAV/$ETF_STARTING_NAV)-1;

            $CURRENT_SP_LEVEL = $S_P_YEAR_START_VALUE * (1+ $YTD_SP_RETURN);

            $SPX_PERIOD_RETURN = ($CURRENT_SP_LEVEL/$S_P_REFERENCE_VALUE) -1;
                

            if($INDEX_RETURN > 0 ){

                $REMAINING_BUFFER = ($INDEX_RETURN - $TOTAL_BUFFER)+(($ETF_STARTING_NAV - $ETF_CURRENT_NAV)/ $ETF_CURRENT_NAV) + $TREASURY_YIELD;
            
            }elseif($INDEX_RETURN <= 0 && $INDEX_RETURN > $TOTAL_BUFFER){

                $REMAINING_BUFFER = (($ETF_CURRENT_NAV - $ETF_CURRENT_NAV)/ $ETF_CURRENT_NAV )+ $$TREASURY_YIELD;
            ...
            $DOWNSIDE_BEFORE_BUFFER = $SPX_PERIOD_RETURN >= 0 ? '0.00' : $SPX_PERIOD_RETURN;

            $DATE_DIFF = strtotime($OUTCOME_END_DATE) - time();
                        ...
                        <table class="table">
                            <thead>
                                <tr>
                                    ...
                                    <th class="table-subhead pr-0" colspan="3" align="right">As of {{ date('m/d/Y', strtotime($OUTCOME_START_DATE))}}</th>
                                    <!-- <th class="table-subhead pr-0" colspan="3" align="right">As of 6/1/2020</th> -->
                                </tr>
                            </thead>
                            <tbody>
                                <tr class="nobg subhead-row">
                                    ...
                                    <th scope="col" class="text-center text-nowrap">Downside Before Buffer</th>
                                ...
                                <tr>
                                    ...
                                    <!-- <td class="text-center">${{round($ETF_CURRENT_NAV, 2)}}/{{round($PERIOD_RETURN, 2)}}%</td>
                                    <td class="text-center">{{ round($SPX_PERIOD_RETURN,2) }}%</td>
                                    <td class="text-center">{{ round($REMAINING_BUFFER,2) }}%</td>
                                    <td class="text-center">{{ round($DOWNSIDE_BEFORE_BUFFER,2) }}%</td>
                                    <td class="text-center">{{ (int)$REMAINING_OUTCOME_PERIOD }} days</td> -->
                                </tr>
        ...
        <div class="data-row mb-0 animContent" data-sr-id="10"
            ...
            <div class="table-responsive">
                <table class="table">
                    <thead>
                        <tr>
                            <th scope="col" colspan="5" class="table-head">TOP 10 HOLDINGS</th>
                        </tr>
                    </thead>
                    <tbody>

angular.module('truemarkApp').controller('aboutUsController', ['$http', 'myConfig', '$location', '$routeParams', '$scope', '$timeout', 'Upload', function ($http, myConfig, $location, $routeParams, $scope, $timeout, Upload) {
    $scope.dataFrm = {};
    ...
    $scope.submit = function () {
        ...
        $scope.dataFrm.call = 'aboutus';
        $scope.dataFrm.stype = 'saveData';
        $scope.dataFrmError = {};
        $http({
            method: 'POST',
            url: 'cms/aboutUs/save',
            data: $scope.dataFrm
            ...
            if (err.status == 422) {
                ...
                $("html,body").animate({
                    scrollTop: $(".about-" + Object.keys($scope.dataFrmError)[0]).offset().top - ($("header").height() + 10)
                }, 1000);
            }
        });
    };

angular.module('truemarkApp').controller('resourceDisclosureController', ['$http', 'myConfig', '$location', '$routeParams', '$scope', '$timeout', 'Upload', function($http, myConfig, $location, $routeParams, $scope, $timeout, Upload){
      $scope.dataFrm = {};
      ...
      var promise = $http({method: 'POST', url: 'cms/resourceDisclosure/edit', data: dataToSend});
      ...
      promise.then(
          function(payload) { 
              $scope.dataFrm = payload.data.DATA; 
                  ...
                  $scope.dataFrm.meta_keyword = payload.data.DATA.meta_keyword ? payload.data.DATA.meta_description : '';
    ...
    $scope.submit = function() {
        ...
        $http({method: 'POST', url: 'cms/resourceDisclosure/save', data: $scope.dataFrm}).success(function(response){
    ...
    $scope.removeFile = function(field, type, $event) {
        ...
        if(c) {
            $http({method: 'POST', url: 'cms/resourceDisclosure/removeBanner', data: {call: 'resource', stype:type, field:field}}).success(function(response){
                //console.log(response);  
              if (response.SUCCESS== 1) {
